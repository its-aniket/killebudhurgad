      ...
      <main>
        {story.map((chapter, index) => (
          <section key={index} id={index === 1 ? "process" : index === 2 ? "certifications" : undefined}
            className={`scroll-mt-24 overflow-hidden py-16 sm:py-24 ${index === 3 ? "bg-[#2D5F2E] text-white" : index % 2 === 0 ? "bg-[#F4F5ED]" : "bg-white"}`}>
            <div className={`mx-auto max-w-7xl px-6 lg:px-10 ${index === 3 ? "max-w-4xl text-center" : "grid items-center gap-10 lg:grid-cols-2 lg:gap-16"}`}>
              <div>
                ...
                <p className={`text-base leading-8 ${index === 3 ? "text-white/80" : "text-gray-600"}`}>{chapter.body}</p>
              ...
              {index === 1 && <div className="rounded-3xl border border-[#D4A574]/30 bg-[#F7F8F4] p-8 sm:p-12"><p className="font-['Poppins'] text-2xl font-semibold leading-relaxed text-[#2D5F2E]">{chapter.callout}</p></div>}
