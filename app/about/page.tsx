      ...
      <main>
        {story.map((chapter, index) => (
          <section key={index} id={index === 1 ? "process" : index === 2 ? "certifications" : undefined}
            className={`scroll-mt-24 overflow-hidden py-16 sm:py-6 ${index === 3 || index === 1 ? "bg-[#2D5F2E] text-white" : index % 2 === 0 ? "bg-[#F4F5ED]" : "bg-white"}`}>
            <div className={`mx-auto max-w-7xl px-6 lg:px-10 ${index === 3 ? "max-w-4xl text-center" : "grid items-center gap-10 lg:grid-cols-2 lg:gap-16"}`}>
              <div>
                ...
                {index === 0 ? (
                  <h1 className="font-['Poppins'] text-4xl font-bold leading-tight sm:text-6xl lg:text-6xl">{chapter.title}</h1>
                ) : (
              ...
              {index === 1 && <div className="rounded-3xl border border-white/20 bg-white/10 p-8 sm:p-16"><p className="font-['Poppins'] text-2xl font-semibold leading-relaxed text-white">{chapter.callout}</p></div>}
